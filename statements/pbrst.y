...
void check_introduction_passage(char *passage, char *az);
void check_cover(double cover);
void check_unique_prepare();
void check_fragment(char *passage, char *az_nt, char *az_ot);
...
introduction
    : INTRODUCTION passage A_Z FORM AZLITERAL opt_introduction_explanations {
...
fragment
    : FRAGMENT passage A_Z FORM AZLITERAL
        MATCHES ot_passage A_Z FORM AZLITERAL opt_unique
...
check_rawposition_length(char *s)
{
  ...
  if (strstr(s, "length") == NULL) {
    add_parseinfo("%d,%d: warning: no length is given, consider adding it W1\n", yylineno, yycolumn);
    ...
    if (to-from+1 != length) {
      if (correct_raw == 1) {
        ...
        add_parseinfo("%d,%d: error: correctable, inconsistent length: %d-%d+1!=%d E1\n", yylineno, yycolumn, to, from, length);
      }
    } else {
      add_parseinfo("%d,%d: info: consistent length: %d-%d+1==%d I1\n", yylineno, yycolumn, to, from, length);
    }
  }
...
check_nt_passage(char *book, char *info, char *verse)
{
  ...
  if (strstr(l, "error: ") != NULL) {
    ...
    add_parseinfo("%d,%d: info: `lookup1 %s %s %s` = %s I2\n", yylineno, yycolumn, book, info, verse, l);
  }
  ...
  if (strstr(r, "error: ") != NULL) {
    ...
    add_parseinfo("%d,%d: info: `raw %s %s %d %d` = %s I3\n", yylineno, yycolumn, book, info,
  ...
  if (!err && strcmp(l, r)==0) {
    ...
    if (correct_raw == 1) {
      ...
      add_parseinfo("%d,%d: corrected: results of lookup and raw did not match\n", yylineno, yycolumn);
        ...
        if (!err)
          add_parseinfo("%d,%d: error: correctable, results of lookup and raw do not match E2\n", yylineno, yycolumn);
        else
          add_parseinfo("%d,%d: error: results of lookup and raw do not match E3\n", yylineno, yycolumn);
      }
    }
...
void
check_introduction_passage(char *passage, char *az)
{
  extern int yylineno;
  extern int yycolumn;
  char *l;
  ...
  if (strstr(l, "error: ") != NULL) {
    add_parseinfo("%d,%d: %s\n", yylineno, yycolumn, l);
    err = true;
    strcpy(l, az); // workaround, maybe the az information is correct
  } else {
    add_parseinfo("%d,%d: info: `lookup1 %s %s %s` = %s I2\n", yylineno, yycolumn, nt_book, nt_info, passage, l);
  }

  if (strcmp(l, az) == 0)
    add_parseinfo("%d,%d: info: introduction %s matches to a-z form %s I5\n", yylineno, yycolumn, passage, az);
  else {
    add_parseinfo("%d,%d: error: introduction %s does not match to a-z form %s, it should be %s E5\n", yylineno, yycolumn, passage, az, l);
    }
  // Get passage as raw text:
  char *r;
  int length = intervals[iv_counter-1][1] - intervals[iv_counter-1][0] +1;
  ...
  for (int i=0; i<substrings; i++) {
    ...
    do {
      ...
      if (strstr(az, s) != NULL) {
        add_parseinfo("%d,%d: info: substring %s found in introduction I6\n", yylineno, yycolumn, s);
        }
      else {
       add_parseinfo("%d,%d: error: substring %s not found in introduction E6\n", yylineno, yycolumn, s);
       }
      s = next+1;
    } while (next != NULL);
  }
...
void
check_fragment(char *passage, char *az_nt, char *az_ot) {
  extern int yylineno;
  extern int yycolumn;
  ...
  if (strstr(ln, "error: ") != NULL) {
    ...
    err = true;
    strcpy(ln, az_nt); // workaround, maybe the az information is correct
  } else {
    add_parseinfo("%d,%d: info: `lookup1 %s %s %s` = %s I2\n", yylineno, yycolumn, nt_book, nt_info, passage, ln);
  }
  if (strcmp(ln, az_nt) == 0)
    add_parseinfo("%d,%d: info: NT fragment %s matches to a-z form I7\n", yylineno, yycolumn, passage);
  else
    add_parseinfo("%d,%d: error: NT fragment %s does not match to a-z form %s, it should be %s E7\n", yylineno, yycolumn, passage, az_nt, ln);
  ...
  if (strstr(lo, "error: ") != NULL) {
    ...
    strcpy(lo, az_ot); // workaround, maybe the az information is correct
  } else {
    add_parseinfo("%d,%d: info: `lookup1 %s %s %s` = %s I2\n", yylineno, yycolumn, ot_book, ot_info, ot_verse, lo);
  }
  if (strcmp(lo, az_ot) == 0)
    add_parseinfo("%d,%d: info: OT fragment %s matches to a-z form I8\n", yylineno, yycolumn, ot_passage);
  else
    add_parseinfo("%d,%d: error: OT fragment %s does not match to a-z form %s, it should be %s E8\n", yylineno, yycolumn, ot_passage, az_ot, lo);
  // add_parseinfo("%d,%d: debug: parsed ot_passage=%s ot_info=%s ot_book=%s ot_verse=%s az_ot=%s diff=%f\n", yylineno, yycolumn, ot_passage, ot_info, ot_book, ot_verse, az_ot, difference);
  double jd = jaccard_dist1(ln, lo);
  if (fabs(jd-difference) <= EPS)
    add_parseinfo("%d,%d: info: fragments differ by %4.2f%% I9\n", yylineno, yycolumn, difference * 100.0);
  else {
    if (correct_differ == 1) {
      add_parseinfo("%d,%d: corrected: fragments in reality differed by %4.2f%%\n", yylineno, yycolumn, jd * 100.0);
      difference = jd; // correction
    } else
      add_parseinfo("%d,%d: error: correctable, fragments in reality differ by %4.2f%% E9\n", yylineno, yycolumn, jd * 100.0);
    }
...
#define MAX_FOUND 10
  ...
  if (count == MAX_FOUND)
    add_parseinfo("no, at least %d occurrences I10\n", count);
  else if (count>1)
    add_parseinfo("no, %d occurrences I10\n", count);
  else
    add_parseinfo("yes I11\n");
  if (count==1 && !unique_prep) {
    add_parseinfo("%d,%d: warning: OT passage is unique, consider mentioning it W2\n", yylineno, yycolumn);
  }
  if (count>1 && unique_prep) {
    add_parseinfo("%d,%d: error: OT passage is not unique E10\n", yylineno, yycolumn);
  }
  unique_prep = false;
  free(ln);
  free(lo);
...
void check_cover(double cover) {
  ...
  if (fabs(real_cover-cover) <= 100*EPS) {
    add_parseinfo("%d,%d: info: cover %4.2f%% is correct I12\n", yylineno, yycolumn, cover);
  } else {
    if (correct_cover == 1) {
      ...
      add_parseinfo("%d,%d: error: correctable, cover %4.2f%% is incorrect (union length: %d, covered: %d), in reality %4.2f%% E11\n",
        yylineno, yycolumn, cover, union_length, covered, real_cover);
  }
       ...
       for (int j=i+1; j<iv_counter; j++) {
         ...
         if (ftype == NT_FRAGMENT || ftype == NT_INTRODUCTION) {
           ...
           if (!(nt_intro_start > fend || nt_intro_end < fstart)) {
             if (ftype == NT_FRAGMENT)
               add_parseinfo("%d,%d: error: NT introduction interval %d overlaps fragment interval %d E12\n",
                 yylineno, yycolumn, i, j);
             if (ftype == NT_INTRODUCTION)
               add_parseinfo("%d,%d: error: NT introduction intervals %d and %d overlap each other E13\n",
                 yylineno, yycolumn, i, j);
           overlap_error = true;
           }
         }
  ...
  if (!overlap_error) {
    add_parseinfo("%d,%d: info: NT overlap check done I13\n", yylineno, yycolumn);
  }
  ...
  if (!(nt_headline_start == imin && nt_headline_end == imax)) {
    if (nt_headline_start <= imin && nt_headline_end >= imax) {
    add_parseinfo("%d,%d: warning: NT headline interval [%d,%d] contains NT fragments union [%d,%d] but they do not match W3\n",
      yylineno, yycolumn, nt_headline_start, nt_headline_end, imin, imax);
    }
    else {
      add_parseinfo("%d,%d: error: NT headline interval [%d,%d] does not contain NT fragments union [%d,%d] E14\n",
        yylineno, yycolumn, nt_headline_start, nt_headline_end, imin, imax);
    }
  } else {
  ...
  for (int i=1; i<=max_ot_headline; i++) {
    ...
    add_parseinfo(", union: [%d,%d]\n", oimin, oimax);
    add_parseinfo("%d,%d: info: OT headline %d %s %s has at least one corresponding fragment I15\n", yylineno, yycolumn,
        i, books_s[i], infos_s[i]);
    ...
    if (!(ot_headline_start == oimin && ot_headline_end == oimax)) {
      ...
      add_parseinfo("%d,%d: warning: OT headline interval %d [%d,%d] contains OT fragments union [%d,%d] but they do not match W4\n",
        ...
        add_parseinfo("%d,%d: error: OT headline interval %d [%d,%d] does not contain OT fragments union [%d,%d] E16\n",
          yylineno, yycolumn, i, ot_headline_start, ot_headline_end, oimin, oimax);
      }
    } else {
      add_parseinfo("%d,%d: info: OT headline interval %d check done I16\n", yylineno, yycolumn, i);
    }
    ...
    for (int j=fragments_start + 1; j<iv_counter; j++) { // register each fragment...
      ...
      if (ftype == OT_PASSAGE && strcmp(books_s[i], books_s[j])==0 && strcmp(infos_s[i], infos_s[j])==0) {
        ...
        if (ot_overlap) {
          add_parseinfo("%d,%d: error: OT intervals %d and %d overlap E17\n",
            yylineno, yycolumn, ot_overlap_i, j);
        }
      }
    }
    if (!ot_overlaps)
      add_parseinfo("%d,%d: info: OT interval check for OT headline %d done I17\n",
            yylineno, yycolumn, i);
...
// Taken from https://stackoverflow.com/a/32496721/1044586
char* replace_char(char* str, char find, char replace){
