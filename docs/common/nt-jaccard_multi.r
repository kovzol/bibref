pdf(file='nt-jaccard-multi-Rplots.pdf');
library(sm); data = NULL; group = NULL;
jaccard_Matthew <- read.csv('nt-jaccard_Matthew.csv', header = F);
sm.density(jaccard_Matthew,xlim=c(0,1),ylim=c(0,3),col='red',lty=1,xlab='',ylab='');
par(new=TRUE);
jaccard_Romans <- read.csv('nt-jaccard_Romans.csv', header = F);
sm.density(jaccard_Romans,xlim=c(0,1),ylim=c(0,3),col='blue',lty=1,xlab='',ylab='');
par(new=TRUE);
jaccard_Hebrews <- read.csv('nt-jaccard_Hebrews.csv', header = F);
