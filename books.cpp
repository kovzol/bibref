...
{
    virtual const char *what() const throw() { return "Invalid module."; }
} InvalidModuleException;

class InvalidBookException : public exception
{
    virtual const char *what() const throw() { return "Invalid book."; }
} InvalidBookException;

class InvalidVerseException : public exception
{
    virtual const char *what() const throw() { return "Invalid verse."; }
} InvalidVerseException;

class InvalidPassageException : public exception
{
    virtual const char *what() const throw() { return "Invalid passage."; }
...
string processWord(string word)
{
    ...
    for (int i = 0; i < word.length(); i += 2) {
        ...
        if (c1
            == 'c') { // Some systems do not support greek ϲ so we fall back sometimes to latin c in the input
            i--;
        ...
        if (c1 == 0xCE || c1 == 0xCF) {
            // force lowercase
            if (c1 == 0xCE && c2 < 0xB1 && !(c2 >= 0xA0 && c2 <= 0xA9)) {
...
{
    if (moduleName == "SBLGNT" || moduleName == "StatResGNT")
        return true;
    return false;
}
...
int addBooks_cached(string moduleName, string installedModuleVersion)
{
    ...
    for (vector<string>::iterator i = bookNames.begin(); i != bookNames.end(); ++i) {
        ...
        std::ifstream tokensFile(path + "/" + bookName
                                 + ".tokens"); // open tokens for the given book
        vector<int> tokens;
        int t;
        while (tokensFile >> t) {
        ...
        try {
            ...
            // cout << bookName << endl;
            while (
                fscanf(verseFile, "%7s %d %d %d %d", reference, &start, &end, &tokensStart, &tokensEnd)
                != EOF) {
                if (bookName == "Psalms") {
                    vector<string> r;
                ...
                book.addVerse(start,
                              ...
                              string(reference),
                              tokensStart,
                              tokensEnd - tokensStart + 1);
            }
        } catch (exception &e) {
            error("Data loading error from cache file " + verseFileName + ".");
            error("Cache may be corrupt or incompatible with this version. Consider removing it, "
                  "then retry.");
...
int addBooks(string moduleName, string firstVerse, string lastVerse, bool removeAccents)
{
    ...
    for (long v = bookStart; v <= bookEnd; ++v) {
        ...
        if (verse.size() > 0) {
            ...
            if (reference.rfind("0:", 0) != 0
                && !boost::algorithm::ends_with(reference,
                                                ":0")) { // we don't want a 0th chapter or a 0th verse
                if (lastBookName.compare(bookName) != 0) {
                    ...
                    if (create_cache) { // Create the cache if it is possible. Open the raw book and tokens.
                        FILE *lastBookFile = fopen((path + "/" + lastBookName + ".book").c_str(),
                                                   "wa");
...
#endif
                    info(lastBookName + " contains " + to_string(lastBookText.length())
                         + " characters,");
                    add_vocabulary_item(
                    ...
                    if (create_cache) {
                        ...
                        lastBookVersesFile = fopen((path + "/" + lastBookName + ".verses").c_str(),
                                                   "wa");
                    }
                } else {
...
#endif
                    info("and " + lastBookName + " contains "
...
Fingerprint getTextFingerprint(const string &book,
                               const string &info,
                               const string &start,
                               const string &end,
                               int startOffset,
...
int compare(const string &book1,
            const string &info1,
            const string &verseInfo1s,
            const string &verseInfo1e,
            int startOffset1,
            int endOffset1,
            const string &book2,
            const string &info2,
            const string &verseInfo2s,
            const string &verseInfo2e,
            int startOffset2,
            int endOffset2)
{
    ...
    Fingerprint f1
        = getTextFingerprint(book1, info1, verseInfo1s, verseInfo1e, startOffset1, endOffset1);
    Fingerprint f2
...
string getText(const string &book,
               const string &info,
               const string &verseInfoS,
               const string &verseInfoE,
               int startOffset,
               int endOffset)
{
    Book b = getBook(book, info);
...
vector<string> find_min_unique(string text, const string &moduleName, bool verbose)
{
    ...
    int extreme_limit
        = 50000; // above this there is no hope to get a result because of out of memory error
    vector<string> retval;
    int l = text.length();
    if (l > extreme_limit) { // warn the user, but don't stop
        error("Input is extremely long (" + to_string(l)
              + " characters), expect out of memory error.");
...
string _extend(const string &moduleName1,
               ...
               bool verbose)
{
    ...
    while (citation && pos1S > 0 && pos2S > 0) { // shift left in both Bible editions
        pos1S--;
        pos2S--;
        citation = text1.at(pos1S)
                   == text2.at(pos2S); // until there is no plausible verbatim citation anymore
    }
    if (!citation) {
    ...
    while (citation && pos1E < text1.length() - 1
           && pos2E < text2.length() - 1) { // shift right in both editions
        pos1E++;
        pos2E++;
    ...
    if (verbose) {
        info("Extended match is " + ot_color + moduleName1 + " " + book1 + " " + verse1infoS + " "
             ...
             + verse2infoS + " " + verse2infoE + reset_color + " (" + ot_color
             + text1.substr(pos1S, pos1E - pos1S + 1) + reset_color + ", length "
             + to_string(pos1E - pos1S + 1) + ")."); // report info verbosely
    }
    return ot_color + moduleName1 + " " + book1 + " " + verse1infoS + " " + verse1infoE
           + reset_color + " = " + nt_color + moduleName2 + " " + book2 + " " + verse2infoS + " "
           + verse2infoE + reset_color + "," + to_string(pos1S) + "," + to_string(pos1E - pos1S + 1)
           + "," + to_string(pos2S); // return concise report
}

void extend(const string &moduleName1,
            const string &moduleName2,
            const string &book2,
...
void getrefs(const string &moduleName2,
             ...
             int end)
{
    ...
    vector<string> minunique = find_min_unique(text,
                                               moduleName1,
                                               0); // get all minimally unique subtexts of the input
    for (string m : minunique) {                   // For each subtext...
        vector<string> found
            = find_all(m,
                       moduleName2,
                       maxresults); // find all plausible verbatim quotations in the other edition
        for (string f : found) {    // For each candidate...
            ...
            int pos = stoi(
                info1[1]); // pos stores the position of plausible verbatim quotation in the other edition
            string ext = _extend(moduleName1,
                                 moduleName2,
                                 book2,
                                 pos,
                                 pos + m.length() - 1,
                                 0); // Extend the plausible quotation...
    ...
    sort(refs.begin(),
         refs.end(),
         compareReference); // Sort the results by text length (of the extension).
