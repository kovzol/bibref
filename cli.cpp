#define BIBREF_VERSION "2025Feb04"
...
string errorLatintextIncomplete = "Either " + latintextCmd + "1 or " + latintextCmd
                                  + "2 must be used.";
...
string getHelp(const string &key)
{
    vector<string> helpStr = {
        "* `addbooks`: Load the books of LXX, SBLGNT and StatResGNT (if any of these are "
        "available). "
        "Most commands require that these books are already loaded. After using this command for "
        "the "
        ...
        "* `lookup` *Bible* *book* *verse*: Search for the given *verse* in the given *book* in "
        "the "
        ...
        "* `compare12`: Compare the two clipboards with a 2-long substring-fingerprint "
        "(2-shingles) "
...
void processLengthCmd(string input)
{
    ...
    info("Length of text " + to_string(index + 1) + " is " + to_string(text[index].length()) + ".");
    return;
}
...
void processPsalminfoCmd(string input)
{
    ...
    if (size == 3) {
        try {
            ...
            info(tokens[1] + " Psalm " + tokens[2] + " contains " + to_string(psalmInfo)
                 + " verses.");
        } catch (exception &e) {
            error(errorMisc);
        }
